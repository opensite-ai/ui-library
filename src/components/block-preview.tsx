...
function buildPreviewBlocks(block: Block): PageSpeedBlock[] {
  return [
    {
...
function createShowcaseRenderer(
  ...
  return function ShowcasePreviewRenderer({ block }) {
    return (
      <PreviewComponent
        {...((block.blockProps as Record<string, unknown> | undefined) ?? {})}
      />
    );
  };
}
...
export function BlockPreview({ block, className }: BlockPreviewProps) {
  ...
  const [activeRendererType, setActiveRendererType] = useState<string | null>(
    null,
  );

  const PreviewComponent = useMemo(() => getComponent(block.id), [block.id]);
  const previewBlocks = useMemo(() => buildPreviewBlocks(block), [block]);
  const rendererType = useMemo(() => `showcase:${block.id}`, [block.id]);
  const isRendererReady =
    !!PreviewComponent && activeRendererType === rendererType;
  const error = PreviewComponent ? null : "Component not registered";
  const isLoading = !mountNode || !isRendererReady;
...
<html lang="en">
  ...
  useEffect(() => {
    ...
    registerBlockRenderer(
      rendererType,
      createShowcaseRenderer(PreviewComponent),
    );

    const timeoutId = window.setTimeout(() => {
      setActiveRendererType(rendererType);
    }, 0);

    return () => {
      window.clearTimeout(timeoutId);
      unregisterBlockRenderer(rendererType);
    };
  }, [PreviewComponent, rendererType]);
  ...
  useEffect(() => {
    if (!mountNode || !PreviewComponent || !isRendererReady) return;
  ...
  }, [isRendererReady, mountNode, PreviewComponent, syncCssToIframe]);
  ...
  const activeCaptureHeight = viewMode === "capture" ? captureHeight : null;
  ...
  const containerStyle = useMemo(() => {
    ...
    return {
      ...
      height: isCapture
        ? activeCaptureHeight
          ? `${activeCaptureHeight}px`
          : "600px"
        : undefined,
    } as CSSProperties;
  }, [activeCaptureHeight, viewMode]);
    ...
    <div className={cn("space-y-4", className)}>
      ...
      >
        ...
        {mountNode &&
          ...
          isRendererReady &&
          createPortal(
            <EnhancedBlocksRenderer blocks={previewBlocks} />,
            mountNode
          )}
      </div>
